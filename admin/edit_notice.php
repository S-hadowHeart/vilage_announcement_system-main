...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title_en && $desc_en && $category_id) {
        $stmt = $db->prepare('UPDATE notices SET title_en=?, title_hi=?, title_gu=?, description_en=?, description_hi=?, description_gu=?, category_id=?, published_at=?, expiry_at=? WHERE id=?');
        $stmt->execute([$title_en, $title_hi, $title_gu, $desc_en, $desc_hi, $desc_gu, $category_id, $published_at, $expiry_at, $id]);
        log_action($admin_id, 'edit_notice', $title_en, $db);
        header('Location: notices.php');
        exit;
    } else {
...
<body class="theme-bg min-h-screen">
    <div class="max-w-xl mx-auto p-4">
        <h1 class="text-2xl font-bold theme-text mb-4">Edit Notice</h1>
        <?php if ($error): ?>
        ...
        <form method="post" class="space-y-4 bg-white p-4 rounded shadow">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <div>
                    ...
                    <input type="text" name="title_en" class="w-full border rounded px-2 py-1" value="<?= htmlspecialchars($notice['title_en']) ?>" required>
                </div>
                <div>
                    <label class="block font-semibold">Title (HI)</label>
                    <input type="text" name="title_hi" class="w-full border rounded px-2 py-1" value="<?= htmlspecialchars($notice['title_hi']) ?>">
                </div>
                <div>
                    <label class="block font-semibold">Title (GU)</label>
                    <input type="text" name="title_gu" class="w-full border rounded px-2 py-1" value="<?= htmlspecialchars($notice['title_gu']) ?>">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <div>
                    <label class="block font-semibold">Description (EN)</label>
                    <textarea name="description_en" class="w-full border rounded px-2 py-1" required><?= htmlspecialchars($notice['description_en']) ?></textarea>
                </div>
                <div>
                    <label class="block font-semibold">Description (HI)</label>
                    <textarea name="description_hi" class="w-full border rounded px-2 py-1"><?= htmlspecialchars($notice['description_hi']) ?></textarea>
                </div>
                <div>
                    <label class="block font-semibold">Description (GU)</label>
                    <textarea name="description_gu" class="w-full border rounded px-2 py-1"><?= htmlspecialchars($notice['description_gu']) ?></textarea>
                </div>
            </div>
            <div>
                ...
                <select name="category_id" class="w-full border rounded px-2 py-1" required>
                    ...
                    <?php foreach ($categories as $cat): ?>
                        <option value="<?= $cat['id'] ?>" <?= $notice['category_id']==$cat['id']?'selected':'' ?>> <?= htmlspecialchars($cat['name_en']) ?> </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <input type="date" name="published_at" class="w-full border rounded px-2 py-1" value="<?= date('Y-m-d', strtotime($notice['published_at'])) ?>">
                </div>
                <div>
                    <label class="block font-semibold">Expiry Date</label>
                    <input type="date" name="expiry_at" class="w-full border rounded px-2 py-1" value="<?= date('Y-m-d', strtotime($notice['expiry_at'])) ?>">
                </div>
            </div>
            <button type="submit" class="theme-btn px-4 py-2 rounded">Save Changes</button>
